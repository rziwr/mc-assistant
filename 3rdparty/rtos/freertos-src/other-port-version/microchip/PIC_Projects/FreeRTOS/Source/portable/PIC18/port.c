...
#include "mplabc18\v3.39\h\timers.h"
#include "compare.h"
...
#define portTIMER_FOSC_SCALE			( ( unsigned portLONG ) 8 )
...
/*********************************************************************************************************************** */
/*!
 *  \brief    Here is the real "meat" to set up timers for a regular tick
 *
 *  \return   
 *
 *  \remarks  If you change the #if 0 to #if 1 you should get what I call a "simple" timer
 *            .instead of the capture/compare stuff.
 *            .The disadvantage of a "simple" timer would be precision.
 *            .The advantage of a "simple" timer would be reduced complexity and better portability.
 *            I'm __NOT__ using NOR masks, since they seem to be buggy, so don't define USE_OR_MASKS
 */
/*********************************************************************************************************************** */
/*$on*/
static void prvSetupTimerInterrupt (void)
{
    const unsigned portLONG ulConstCompareValue =
        (
            (configCPU_CLOCK_HZ / portTIMER_FOSC_SCALE) /
            configTICK_RATE_HZ
        );
    unsigned portLONG ulCompareValue;
    unsigned portCHAR ucByte;
    unsigned char config = 0x00, config1 = 0x00;
    #if USE_SIMPLE_TIMER > 0

    /* simple timer int only */
        #ifndef USE_OR_MASKS

    /* use and masks */
    config = TIMER_INT_ON & T0_8BIT & T0_SOURCE_INT & T0_EDGE_FALL & T0_PS_1_1;
        #else

    /* use or masks */
            #error You are on thin ice here - NOR masks might work, might not
    config = TIMER_INT_ON | T0_8BIT | T0_SOURCE_INT | T0_EDGE_FALL | T0_PS_1_1;
        #endif /* or masks */
    OpenTimer0(config);
    WriteTimer0(-2345);
    ...
    #else /* USE_SIMPLE_TIMER > 0 */

    /* capture compare timer + timer3 */
    config = 0;
    config1 = 0;
    config = TIMER_INT_OFF & T3_16BIT_RW & T3_SOURCE_FOSC & T3_PS_1_8 & T3_SYNC_EXT_OFF & T3_OSC1EN_OFF;
    OpenTimer3(config, config1);
    config = 0;
    config = COM_INT_ON & COM_TRIG_SEVNT;
    OpenECompare1(config, (unsigned int) ulConstCompareValue);
    INTCONbits.PEIE = 1;    /* Enable Peripheral Interrupt */
    INTCONbits.GIE = 1;     /* Enable global interrupt */
    #endif
}
